...
+(function (angular) {
  ...
  //TODO: use with caution. if an property of element in array doesn't exist in group, the resultArray may lose some
  // element.
   ...
   * Use this method with event delegate. (attach a event handler on an parent element and listen the special children
   * elements)
  ...
  nyaBsSelect.directive('nyaBsSelect', [
    '$parse', '$document', '$timeout', 'nyaBsConfig', '$compile', function ($parse, $document, $timeout, nyaBsConfig, $compile) {
      ...
      var DROPDOWN_TOGGLE = '<button class="btn btn-default dropdown-toggle" type="button" tabindex="1">' +
        ...
        compile: function nyaBsSelectCompile(tElement, tAttrs) {
          ...
          for (index = 0; index < length; index++) {
            ...
            if (liElement.hasClass('nya-bs-option') || liElement.attr('nya-bs-option')) {
              liElement.find('a').attr('tabindex', '-1');
              // In order to be compatible with old version, we should copy value of value attribute into data-value
              // attribute. For the reason we use data-value instead, see
              // http://nya.io/AngularJS/Beware-Of-Using-value-Attribute-On-list-element/
          ...
          return function nyaBsSelectLink($scope, $element, $attrs, ctrls) {
            ...
            nyaBsSelectCtrl.onCollectionChange = function (values) {
              ...
              /**
               * Behavior change, since 2.1.0, we don't want to reset model to null or empty array when options'
               * collection is not prepared.
               */
              if (Array.isArray(values) && values.length > 0) {
                ...
                if (isMultiple) {
                  ...
                  if (length !== modelValue.length) {
                    // modelValue changed.
                    // Due to ngModelController compare reference with the old modelValue, we must set an new array
                    // instead of modifying the old one. See: https://github.com/angular/angular.js/issues/1751
            ...
            dropdownToggle.on('click', function () {
              ...
              if ($attrs.search === 'true' && $element.hasClass('open')) {
                ...
                if (searchKeyword) {
                  for (index = 0; index < length; index++) {
                    ...
                    if (option.hasClass('nya-bs-option')) {
                      if (!(typeof filter === 'function' ? filter : hasKeyword)(option.find('a'), searchKeyword)) {
            ...
            function findActive() {
              ...
              for (i = 0; i < length; i++) {
                liElement = list.eq(i);
                if (liElement.hasClass('active') && liElement.hasClass('nya-bs-option') && !liElement.hasClass(
                    'not-match')) {
                  return liElement;
                }
              }
            ...
            function findFocus(fromFirst) {
              ...
              if (firstLiElement.hasClass('nya-bs-option') && !firstLiElement.hasClass('disabled') && !firstLiElement.hasClass(
                  'not-match')) {
             ...
             * - if `nya-bs-option` directive is used to populate options and a `value` attribute is specified. use
             * expression of the attribute value.
             * - if `nya-bs-option` directive is used to populate options and no other settings, use the
             * valueIdentifier or keyIdentifier to retrieve value from scope of current nyaBsOption.
              ...
              if (isMultiple && modelValue.length === 0) {
                ...
                $timeout(function () {
                  ...
                  if (isMultiple && $attrs.selectedTextFormat === 'count') {
                    count = 1;
                  } else if (isMultiple && $attrs.selectedTextFormat && (match = $attrs.selectedTextFormat.match(
                      /\s*count\s*>\s*(\d+)\s*/))) {
                  ...
                  if ((typeof count !== 'undefined') && modelValue.length > count) {
                    ...
                    if (localizedText.numberItemSelectedTpl) {
                      ...
                      filterOption.append(document.createTextNode(localizedText.numberItemSelected.replace('%d',
                        modelValue.length)));
                    } else {
  ...
  nyaBsSelect.directive('nyaBsOption', [
    '$parse', function ($parse) {
      ...
      return {
        ...
        compile: function nyaBsOptionCompile(tElement, tAttrs) {
          ...
          /**
           *
           * we want to keep our expression comprehensible so we dont use select as label for value in collection
           */
